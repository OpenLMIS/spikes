function bodyController($scope) {
    ...
    function editRow(){
        var divElement = '<div ng-repeat="col in visibleColumns()" class="ngCell col{{$index}} {{col.cellClass}}" ng-cell><input  name={{$parent.$index}}{{$index}} integer ng-show="edit && isEditableColumn(col.field)" ng-model="row.entity[col.field]" /><span id={{$parent.$index}}{{$index}} style="display:none">Enter Numeric data!</span></div>'+
    ...
    $scope.isEditableColumn = function(field){
        return ($scope.editableColumns.indexOf(field)>=0);
    } ;
